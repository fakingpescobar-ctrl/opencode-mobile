...
object WhisperCpuConfig {
    /**
     * Число потоков whisper. Телефон (Snapdragon 8 Gen 3, 8 ядер) имеет
     * hetero-кластеры: 2 prime-ядра на ~4.6GHz + 6 ядер на ~3.6GHz.
     *
     * Раньше брали ТОЛЬКО 2 prime-ядра (~4.6GHz) — large-v3-turbo (574MB)
     * считал на них 80 сек на фразу. Matmul whisper отлично ложится на все
     * ядра: поднимаем до high-perf (2) + остальные большие, всего до 8.
     *
     * Осторожно: НЕ берём все 8 слепо — слабые efficiency-ядра в барьерах
     * могут замедлить. На 8 Gen 3 остальные 6 ядер всё ещё 3.6GHz, то есть
     * достаточно тяжёлые, поэтому выигрыш гарантирован. Кап 8 защищает.
     */
    val preferredThreadCount: Int
        get() {
            val highPerf = CpuInfo.getHighPerfCpuCount().coerceAtLeast(2)
            val total = Runtime.getRuntime().availableProcessors()
            val extra = (total - highPerf).coerceIn(0, 6)
            return (highPerf + extra).coerceIn(2, 8)
        }
}
