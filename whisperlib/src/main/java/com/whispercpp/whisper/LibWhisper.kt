...
class WhisperContext private constructor(private var ptr: Long) {
    ...
    suspend fun transcribeData(data: FloatArray, printTimestamp: Boolean = true): String = withContext(scope.coroutineContext) {
        ...
        // Поднимаем nice на время счёта: некоторые OEM-прошивки душат фоновые треды до 1-5% CPU.
        android.os.Process.setThreadPriority(android.os.Process.myTid(), android.os.Process.THREAD_PRIORITY_URGENT_AUDIO)
        try {
            WhisperLib.fullTranscribe(ptr, numThreads, data)
        } finally {
            android.os.Process.setThreadPriority(android.os.Process.myTid(), android.os.Process.THREAD_PRIORITY_DEFAULT)
        }
...
private class WhisperLib {
    companion object {
        init {
            ...
            // Форсим обычную CPU-сборку: v8fp16_va виснет в fullTranscribe на этом устройстве.
            Log.d(LOG_TAG, "Loading libwhisper.so (forced, no fp16)")
