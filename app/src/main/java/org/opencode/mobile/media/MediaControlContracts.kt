 ...
 *
 * [exactTrackId] — трек, названный ровно как запрос. [resolvedBy] — чем запрос разобран: `text`
 * или `id`. Вместе они отвечают на вопрос, который сама выдача не отвечает: то ли мы нашли то,
 * что просили, или пришлось выбирать из похожих.
 */
data class MediaSearchResult(
    ...
    val resolvedBy: String = YandexCatalog.RESOLVED_BY_TEXT,
)
...
/**
 * Ответ обхода дерева библиотеки: корень, содержимое узла и честный код результата сессии.
 *
 * Сессия не отдаёт каталожный id играющего трека, и это проверено на устройстве, а не выведено:
 * платформенная сессия прячет индекс активного элемента и id элемента очереди как `@SystemApi`,
 * media3-контроллер отдаёт позиционный счётчик очереди вместо id, `uri` у элемента null, а isrc
 * нет ни у сессии, ни в публичном каталоге. Поэтому сверять «играет ли именно этот трек» с id из
 * mobile_media_search нечем, и проверка идёт по названию. Публиковать же счётчик очереди под
 * именем id опаснее, чем не публиковать ничего: «10» из очереди совпадёт с каталожным id «10»
 * и даст ложное подтверждение.
 */
